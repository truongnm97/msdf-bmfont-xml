...
var charsMap = [
  ...
  transChars = [
    ...
    0x0614 /* ARABIC SIGN TAKHALLUS */, 0x0615 /* ARABIC SMALL HIGH TAH */,
    0x064b /* ARABIC FATHATAN */, 0x064c /* ARABIC DAMMATAN */,
    0x064d /* ARABIC KASRATAN */, 0x064e /* ARABIC FATHA */,
    0x064f /* ARABIC DAMMA */, 0x0650 /* ARABIC KASRA */,
    0x0651 /* ARABIC SHADDA */, 0x0652 /* ARABIC SUKUN */,
    0x0653 /* ARABIC MADDAH ABOVE */, 0x0654 /* ARABIC HAMZA ABOVE */,
    0x0655 /* ARABIC HAMZA BELOW */, 0x0656 /* ARABIC SUBSCRIPT ALEF */,
    0x0657 /* ARABIC INVERTED DAMMA */, 0x0658 /* ARABIC MARK NOON GHUNNA */,
  ...
  theLoop: for (var i = 0; i < apfb.length; ++i) {
    ...
    for (var j = 0; j < combCharsMap.length; ++j) {
      if (
        ...
        toReturn +=
          String.fromCharCode(combCharsMap[j][0][0]) +
...
function generateFallbackGlyphs(charsMap) {
  const fallbackGlyphs = new Map();

  for (var i = 0; i < charsMap.length; i++) {
    for (var j = 1; j < charsMap[i].length; j++) {
      if (charsMap[i][j] != null) {
        fallbackGlyphs.set(
          String.fromCharCode(charsMap[i][j]),
          String.fromCharCode(charsMap[i][0]),
        );
      }
    }
  }
...
exports.fallbackGlyphs = generateFallbackGlyphs(charsMap);
